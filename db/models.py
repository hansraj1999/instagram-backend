...
class User(Base):
    ...
    updated_at = Column(
        DateTime(timezone=True), default=func.utcnow(), on_update=func.utcnow()
    )
    is_verified = Column(Boolean, default=False)
    is_private = Column(Boolean, default=False)
    account_status = Column(
        Enum("ACTIVE", "INACTIVE", "PENDING", name="status_enum"), default="active"
    )
...
class FollowersAndFollowing(Base):
    ...
    created_at = Column(
        DateTime(timezone=True), default=func.utcnow()
    )  # Default timestamp
    is_accepted = Column(
        Boolean, default=False
    )  # Default False for new follow requests
